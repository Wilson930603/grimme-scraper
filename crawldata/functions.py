...
def get_data_db(conn,SQL):
    ...
    cur.close()
    return DATA
def get_cat(CATES,cat_name,cat_link,row):
    cat_link=row.xpath('./button/@data-url').get()
    if cat_name=='':
        cat_name=row.xpath('./button/span[@class="filter-category__link-text "]/text()').get()
    else:
        cat_name+=" ~ "+row.xpath('./button/span[@class="filter-category__link-text "]/text()').get()
    CHK=row.xpath('./filter-category')
    if CHK:
        data=row.xpath('./filter-category/ul/li')
        for rs in data:
            get_cat(CATES,cat_name,cat_link,rs)
    else:
        CATES[cat_name]=cat_link
    return CATES